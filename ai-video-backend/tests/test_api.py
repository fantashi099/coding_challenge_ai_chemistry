...
def test_video_api_lifecycle_and_artifact(tmp_path):
    async def exercise():
        store = JobStore(tmp_path / "jobs.sqlite3")
        transport = httpx.ASGITransport(app=create_app(store=store))
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            created = await client.post("/videos", json={"question": "How does pH work?"})
...
def test_api_validation_and_failure_details(tmp_path):
    async def exercise():
        store = JobStore(tmp_path / "jobs.sqlite3")
        transport = httpx.ASGITransport(app=create_app(store=store))
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            assert (await client.post("/videos", json={"question": "  "})).status_code == 422
            created = await client.post("/videos", json={"question": "A valid question"})
            job_id = created.json()["id"]
            store.claim()
            ...
            assert detail["artifact_url"] is None
